// Importo las librerías necesarias para generar y servir la documentación Swagger
import swaggerJSDoc from 'swagger-jsdoc';
import swaggerUi from 'swagger-ui-express';

// Clase para encapsular la configuración y el inicializador de Swagger
class SwaggerConfig {
  constructor(app) {
    this.app = app; // Guardo la instancia de Express
    this.options = this._getOptions(); // Defino las opciones generales de la API (OpenAPI)
    this.specs = swaggerJSDoc(this.options); // Genero el objeto de especificaciones (JSON) a partir de las opciones
  }

  // Método privado para definir las opciones de la documentación (metadatos, seguridad, etc.)
  _getOptions() {
    return {
      definition: {
        openapi: '3.0.0',
...
        servers: [
          {
            // Uso la variable de entorno para el puerto
            url: `http://localhost:${process.env.PORT || 3000}`,
            description: 'Servidor de desarrollo'
          }
        ],
        components: {
          // Defino el esquema de seguridad JWT (Bearer)
          securitySchemes: {
            bearerAuth: {
              type: 'http',
              scheme: 'bearer',
...
            }
          },
          // Esquemas (Modelos) reusables de datos de entrada y salida
          schemas: this._getSchemas(),
          // Respuestas de error comunes reusables
...
        ]
      },
      // Rutas donde buscar los comentarios JSDoc para generar los endpoints
      apis: [
        './src/routes/*.js',
        './src/controllers/*.js',
        './src/models/*.js'
...
  // Defino los esquemas de modelos de datos (Servicio, Usuario, Login, Paginación, Error, etc.)
  _getSchemas() {
    return {
      Servicio: {
        type: 'object',
...
  // Defino las respuestas de error reutilizables (401, 403, 404, etc.)
  _getResponses() {
    return {
      Unauthorized: {
...
  // Método para inicializar los middlewares de Express y las rutas de documentación
  init() {
...
      },
      // CSS personalizado para estilizar la UI de Swagger
      customCss: `
        .swagger-ui .topbar { display: none }
        .swagger-ui .info hgroup.main h2 { color: #1bc05aff }
...
    // Configuro la ruta /api-docs para servir la UI de Swagger
...
// Exporto la clase para instanciarla en el servidor
